...
becomes available when switching to Codex, Windsurf, Cursor, or any other harness.

Usage: /sync-memory [--dry-run] [--target TARGET] [--list]

Options:
- --dry-run: Preview which files would be written without writing
- --target TARGET: Sync to a specific harness only (codex, windsurf, etc.)
- --list: Show discovered memory files without syncing

Examples:
  /sync-memory
  /sync-memory --dry-run
  /sync-memory --target codex
  /sync-memory --list
