...
class ShimGenerator:
    ...
    def generate_compat_warning_doc(
        self,
        mcp_servers: dict,
        target: str,
        output_path: "Path | None" = None,
    ) -> str:
        """Generate a Markdown compatibility warning document for MCP-incompatible harnesses.

        For harnesses that do not support MCP natively (e.g. aider, cursor),
        this method generates a human-readable Markdown file that lists which
        MCP tools will be absent, what each server provides, and suggested
        manual workarounds.  The file can be placed in the target harness
        config directory so users see it when they open their harness.

        Args:
            mcp_servers: MCP server config dict (from .mcp.json mcpServers).
            target: Target harness name (e.g. "aider", "cursor").
            output_path: If provided, write the doc to this path.

        Returns:
            Markdown string of the compatibility warning document.
        """
        from datetime import datetime as _dt

        # Harnesses with no MCP support at all
        _NO_MCP_HARNESSES = frozenset({"aider", "cursor", "zed", "neovim", "vscode"})
        # Harnesses with partial MCP (some transports only)
        _PARTIAL_MCP_HARNESSES = frozenset({"windsurf", "cline"})

        if target in _NO_MCP_HARNESSES:
            support_level = "none"
        elif target in _PARTIAL_MCP_HARNESSES:
            support_level = "partial"
        else:
            support_level = "full"

        server_list = "\n".join(
            f"- **{name}**: `{cfg.get('command', cfg.get('url', 'unknown'))}`"
            + (f" — {cfg['description']}" if isinstance(cfg, dict) and "description" in cfg else "")
            for name, cfg in mcp_servers.items()
            if isinstance(cfg, dict)
        ) or "_(no MCP servers configured)_"

        if support_level == "none":
            warning_block = (
                f"> **Warning:** `{target}` does not support MCP servers.\n"
                "> The following MCP tools from your Claude Code config are **unavailable** in this harness.\n"
                "> Workflows that depend on these tools will fail silently or produce errors.\n"
            )
            workaround = (
                "## Workarounds\n\n"
                "1. Use Claude Code for tasks that require these MCP tools.\n"
                "2. Install equivalent CLI tools manually and reference them in your task descriptions.\n"
                f"3. For `{target}` sessions, rely on built-in file read/write tools only.\n"
            )
        else:
            warning_block = (
                f"> **Note:** `{target}` has partial MCP support.\n"
                "> Some MCP transports (SSE, HTTP) may require a shim to function correctly.\n"
                "> Run `/sync --shim` to auto-generate transport adapters.\n"
            )
            workaround = (
                "## Workarounds\n\n"
                "1. Run `harnesssync sync --shim` to generate transport shims for incompatible servers.\n"
                "2. Check the `.harnesssync/shims/` directory for generated shim scripts.\n"
            )

        doc = textwrap.dedent(f"""\
            # MCP Server Compatibility Notice

            **Generated by HarnessSync on {_dt.now().strftime("%Y-%m-%d")}**
            **Target harness:** `{target}`
            **MCP support level:** `{support_level}`
            ...
            ---
            *This file was auto-generated by HarnessSync. Do not edit manually.*
            *Re-run `/sync` to update after changing your `.mcp.json`.*
        """)
