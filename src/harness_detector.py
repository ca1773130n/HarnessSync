...
def generate_auto_config(
    ...
    already_configured: list[str] | None = None,
    fmt: str = "json",
) -> str:
    """Generate a ready-to-use HarnessSync config from auto-detected harnesses.

    Scans the system for installed AI coding CLIs and produces a
    ``.harnesssync`` (JSON) or ``harnesssync.toml`` snippet that enables
    sync to all detected harnesses. Users can paste this into their project
    to go from zero-config to fully wired in one step.

    Args:
        detected: Pre-computed detection results (from scan_all_with_versions).
                  If None, auto-detected by calling scan_all_with_versions().
        already_configured: Harness names already in the user's config
                            (excluded from the output to avoid duplicates).
        fmt: Output format — "json" (default) or "toml".

    Returns:
        Formatted config string ready to write to .harnesssync or harnesssync.toml.
    """
    import json as _json
    ...
    already_set = set(already_configured or [])
    new_targets = sorted(
        name for name in detected if name not in already_set
    )
    ...
    unsupported = [t for t in new_targets if t not in _SUPPORTED]

    if fmt == "toml":
        lines = [
            "# Auto-generated by HarnessSync harness detector",
            "# Add to harnesssync.toml in your project root",
            "",
            "[sync]",
        ]
        if supported_new:
            target_str = ", ".join(f'"{t}"' for t in supported_new)
            lines.append(f"targets = [{target_str}]")
        else:
            lines.append('targets = []  # no supported harnesses detected')
        lines.append("")
        for target in supported_new:
            info = detected[target]
            ver = info.get("version") or "unknown"
            lines.append(f"[targets.{target}]")
            lines.append("enabled = true")
            lines.append(f"# Detected v{ver}")
            lines.append("")
        if unsupported:
            lines.append(
                f"# Detected but not yet supported by HarnessSync: "
                f"{', '.join(unsupported)}"
            )
        return "\n".join(lines)
    else:
        # JSON format (.harnesssync)
        config: dict = {}
        if supported_new:
            config["only_targets"] = supported_new
        else:
            config["only_targets"] = []
        config["_auto_detected"] = {
            name: {
                "version": detected[name].get("version"),
                "in_path": detected[name].get("in_path", False),
                "config_dir": detected[name].get("config_dir", False),
            }
            for name in supported_new
        }
        if unsupported:
            config["_unsupported_detected"] = unsupported
