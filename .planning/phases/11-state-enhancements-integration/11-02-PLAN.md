...
must_haves:
  ...
  artifacts:
    ...
    - path: "verify_phase11_integration.py"
      provides: "Integration test for plugin update simulation and full v0.0.2 pipeline"
  key_links:
...
<objective>
Refactor /sync-status to display MCP servers grouped by source (user/project/local/plugin) with scope labels, add plugin drift warnings, and create integration tests verifying the full Phase 11 and v0.0.2 pipeline.

Purpose: Enable users to see at a glance which MCP servers come from plugins vs user configuration, detect plugin version changes, and validate the complete v0.0.2 feature set end-to-end. This satisfies requirements STATE-02 (grouped MCP display) and success criteria #3, #4, #6, #7.

Output: Enhanced /sync-status with MCP source grouping and plugin drift display; integration test script verifying plugin update simulation and full v0.0.2 pipeline.
</objective>
...
<task type="auto">
  <name>Task 2: Integration test for plugin update simulation and full v0.0.2 pipeline</name>
  ...
  <action>
    ...
    **Section 2: Full v0.0.2 Pipeline (Success Criteria #7)**
