...
Purpose: v0.0.1 wrote all MCPs to a single project-level config file. v0.0.2 must route user-scope MCPs to user config paths (`~/.codex/`, `~/.gemini/`) and project-scope MCPs to project config paths (`.codex/`, `.gemini/`). Codex requires env var expansion, Gemini preserves syntax, and unsupported transports must warn.
