...
<task type="auto">
  ...
  <action>
    ...
    1. `_get_enabled_plugins(self) -> set[str]`:
       ...
       - Research note: enabledPlugins field maps plugin_key -> bool (v0.0.2-claude-plugins.md lines 524-533)
    ...
    3. `_get_plugin_mcp_servers(self) -> dict[str, dict]`:
       ...
       - Iterate registry["plugins"] dict: for plugin_key, installs in plugins.items()
         ...
         - Research note: Context7 uses flat format, Figma uses nested format (v0.0.2-claude-plugins.md lines 306-327, 964-978)
