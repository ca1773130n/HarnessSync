...
<task type="auto">
  ...
  <action>
    ...
    1. `_get_user_scope_mcps(self) -> dict[str, dict]`:
       ...
       - IMPORTANT: This replaces the v0.0.1 pattern of reading ~/.mcp.json and ~/.claude/.mcp.json
         The v0.0.1 sources (cc_mcp_global, cc_mcp_claude) were incorrect for user-scope MCPs.
         User-scope MCPs live in ~/.claude.json, NOT in .mcp.json files.
         Research: v0.0.2-claude-plugins.md lines 432-448 confirm ~/.claude.json is user-scope MCP location
    ...
    7. Update `get_source_paths(self) -> dict[str, list[Path]]`:
       ...
       - Remove cc_mcp_global and cc_mcp_claude from MCP source paths (those were v0.0.1 incorrect sources)
...
4. get_mcp_servers() returns flat dict (backward-compatible with v0.0.1 adapters)
