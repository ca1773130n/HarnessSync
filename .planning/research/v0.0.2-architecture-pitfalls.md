# v0.0.2 Architecture & Pitfalls Research: Plugin & MCP Scope Sync
...
**Implication for v0.0.2:** Syncing must implement **lossy conversion** with explicit reporting. Example: Claude Code plugin with 3 hooks, 2 commands, 1 agent, 2 MCP servers → Gemini gets 2 commands, 1 skill (agent converted), 2 MCP servers. **Lost: 3 hooks**. Codex gets 2 MCP servers. **Lost: everything else**.
...
**Implication for v0.0.2:** Cannot sync plugin *distribution metadata* (marketplace.json). v0.0.2 should only sync plugin *capabilities* (hooks → commands, MCP servers). Users must manually publish to Gemini's extension catalog if they want distribution.
...
**Implication for v0.0.2:** HarnessSync cannot automate cross-CLI plugin *installation*, only *configuration sync*. If a Claude Code plugin depends on binary tools (e.g., MCP server is a Python script), those dependencies must be installed separately in Gemini/Codex environments.
...
**Architectural decision required:** v0.0.2 must answer: "What does it mean to sync a Claude Code plugin to Gemini/Codex?"
...
**Implication for v0.0.2:** HarnessSync must detect absolute paths and either:
...
**Recommended approach:** Option 1 (reject) is safest. v0.0.2 should validate MCP server paths and block sync if:
...
- HarnessSync v0.0.1 implementation — Syncs `.mcp.json` config, not MCP server code
...
**Implication for v0.0.2:** MCP server sync is **config-only**. v0.0.2 must:
...
- HarnessSync v0.0.1 decision log (STATE.md #30) — "Conservative sandbox mapping — ANY denied tool -> read-only sandbox mode"
...
**Implication for v0.0.2:** HarnessSync should **never auto-sync MCP servers with dangerous permissions**. Implement safeguards:
...
- HarnessSync v0.0.1 SourceReader implementation (source_reader.py lines 42-53) — Reads both user scope (`cc_home`) and project scope
...
**Current HarnessSync v0.0.1 sync behavior (from source_reader.py):**
...
**Implication for v0.0.2:** Need **scope-aware MCP syncing** that preserves precedence in targets.
...
Current v0.0.1 SourceReader merges scopes. v0.0.2 must:
...
**Confidence:** MEDIUM — Requires architectural change to v0.0.1's merged approach. No direct precedent found in other sync tools.
...
**Problem:** HarnessSync plugin.json declares `"version": "1.0.0"`. If v0.0.2 introduces breaking changes (e.g., new MCP scope-aware sync that's incompatible with v0.0.1 state files), users syncing from v0.0.1 to v0.0.2 may experience failures.

**Implication for v0.0.2:** Must implement **version migration** in StateManager.
...
STATE_FORMAT_VERSION = 2  # v0.0.2 introduces scope-aware MCP tracking
...
def migrate_v1_to_v2(state_v1: dict) -> dict:
    """
    Migrate v0.0.1 state (flat MCP list) to v0.0.2 (scoped MCP).
...
**Confidence:** HIGH — State format migration is standard practice (see HarnessSync v0.0.1 Phase 8 multi-account migration).
...
**Implication for v0.0.2:** Need **feature compatibility matrix** tracked per target.
...
**Implication for v0.0.2:** HarnessSync cannot enforce version pinning (would require package manager integration). **Document limitation.**

**Recommended documentation (for plugin authors):**

```markdown
## v0.0.2 Plugin Sync Limitations
...
**Evidence:**
- HarnessSync v0.0.1 decision log (#30, #33) — "Conservative sandbox mapping," "Never auto-enable yolo mode"
...
**Current v0.0.1 behavior (from STATE.md):**
- Claude "deny" → skip tool entirely in Gemini/Codex
- Never downgrade "deny" to "allow"
- Gemini yolo mode never auto-enabled

**v0.0.2 challenge:** Plugins may bundle MCP servers with **inherent permissions** (e.g., filesystem server grants file access). Syncing the plugin syncs the permission grant.
...
**Implication for v0.0.2:** MCP server permissions are **client-side** (Claude Code enforces), but the server capabilities are **server-side** (server declares tools). Syncing MCP server config to Gemini doesn't sync Claude's permission denials.
...
- HarnessSync v0.0.1 SecretDetector (Phase 5) — Scans environment variables for secrets
...
**v0.0.1 mitigation (from Phase 5):** SecretDetector scans env vars, blocks sync if secrets detected.

**v0.0.2 enhancement required:** Extend SecretDetector to scan **MCP `env` fields**.
...
**Confidence:** HIGH — Secret detection is already implemented in v0.0.1, just needs MCP extension.
...
**Recommendation:** HarnessSync v0.0.2 should maintain separate state tracking for user vs project scopes:
...
**Confidence:** MEDIUM — Requires state format change from v0.0.1.
...
**Evidence:**
- HarnessSync v0.0.1 StateManager (from STATE.md) — SHA256 file hashing for drift detection
...
**Current v0.0.1 behavior:** Hashes all source files, compares to state, only writes if changed.

**v0.0.2 enhancement:** Extend to plugin manifest changes. If plugin.json version unchanged, skip re-syncing plugin capabilities.
...
**Confidence:** HIGH — Already implemented pattern in v0.0.1.
...
- HarnessSync v0.0.1 decision #9 — "Atomic write pattern for state — Tempfile + os.replace prevents corruption"

**Current v0.0.1 behavior:** Atomic file writes via tempfile + os.replace.

**v0.0.2 requirement:** Extend to multi-scope sync. If project scope sync fails, rollback user scope sync.
...
**Relevance to HarnessSync:** v0.0.2 could add `harnesssync status --show-scope` to display where each config came from (user vs project).
...
#### Rollback Strategy 1: Backup-Before-Sync (Current v0.0.1 Approach)

**What:** Before syncing, backup all target files. If sync fails, restore from backup.

**Evidence:**
- HarnessSync v0.0.1 BackupManager (Phase 5) — Creates timestamped backups before sync
...
**Current v0.0.1 implementation (from STATE.md):**
...
**v0.0.2 requirement:** Extend to plugin sync. Backup plugin-installed files (commands, skills, MCP server configs) before syncing new plugin version.
...
**Confidence:** MEDIUM — More complex than v0.0.1 backup approach, but more precise.
...
**Evidence:**
- HarnessSync v0.0.1 commands (README.md) — `/sync --dry-run` already implemented
- install.sh (Phase 7) — `--dry-run` flag for installation preview

**Current v0.0.1 behavior:** DiffFormatter shows what would change without writing.

**v0.0.2 enhancement:** Extend dry-run to plugin sync. Show which capabilities would be synced, which would be skipped.
...
**Confidence:** HIGH — Already implemented in v0.0.1, just needs extension.
...
## Recommendations for v0.0.2 Roadmap
...
**Rationale:** Core v0.0.2 feature. Requires Phase 1 scope-awareness for correct MCP syncing.
...
4. **How to handle plugin updates?** If plugin v0.0.1 synced, then v0.0.2 released with breaking changes, should HarnessSync auto-update synced targets or require manual re-sync? Needs policy decision.
...
2. **Bi-directional sync** — Currently HarnessSync is unidirectional (Claude → targets). Should v0.0.2 support reverse sync (Gemini extension → Claude plugin)?
