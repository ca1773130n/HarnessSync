# v0.0.2 Research Summary: Plugin & MCP Scope Sync
...
v0.0.2 extends HarnessSync to discover MCP servers from installed Claude Code plugins and sync them with proper scope awareness (user/project) to Gemini and Codex. **Critical finding: Gemini extensions are NOT the target** — plugin-provided MCP servers sync to Gemini's `settings.json` mcpServers section (same as v0.0.1 pattern), just with scope and plugin discovery enhancements.
...
### 5. Critical Pitfalls
- **Scope precedence collapse:** v0.0.1 merges flat, losing precedence semantics → fix in v0.0.2
