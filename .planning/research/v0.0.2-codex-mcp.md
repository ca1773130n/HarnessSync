...
**Project:** HarnessSync v0.0.2 — Plugin & MCP Scope Sync
...
This research establishes the foundation for v0.0.2's plugin/MCP scoping features:
...
**Current HarnessSync v0.0.1 behavior:**
- Translates `command` → TOML `[mcp_servers.name]` with `command`/`args`
- Translates `env` → TOML `env` map
- ✅ **No changes needed** — v0.0.1 already handles STDIO correctly
...
### 6.4 Implications for HarnessSync v0.0.2

**CANNOT map Claude Code plugins to Codex plugins** — fundamentally different systems

**Strategy:**
1. Map Claude Code **skills** → Codex skills (already done in v0.0.1)
2. Map Claude Code **plugin-provided MCP servers** → Codex `[mcp_servers]` (NEW for v0.0.2)
3. Map Claude Code **agents** → Codex skills with agent prefix (already done in v0.0.1)
4. **Commands** → Codex skills with cmd prefix (already done in v0.0.1)
...
## 11. Implications for HarnessSync v0.0.2
...
**Recommendation:** Defer to user testing in v0.0.2 beta

---

## 13. Recommended v0.0.2 Approach
